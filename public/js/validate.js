$(function () {
    ...
    $(document).on('paste', 'input', async function (e) {
        let value = $(this).val();
        ...
        if (/\p{Emoji}[^0-9]/u.test(stringPaste)) {
            ...
            if(lastIndex && lastIndex < value.length) {
                $(this).val(stringBefore + '' + stringPaste + '' + stringLast)
                e.preventDefault()
            }else {
                $(this).val(lastIndex ? value : '');
            }
        }
    ...
    $(document).on('input keyup paste change', 'input[data-max]', async function (e) {
        removeErrorInput($(this));
    ...
    $(document).on('input', 'input[data-min-value]', function () {
        if(!$(this).attr('data-min')) return false;
        if($(this).val() < $(this).attr('data-min')) $(this).val($(this).attr('data-min'));
        removeErrorInput($(this));
    })
    ...
    /* VALIDATE PHONE HOTLINE */
    $(document).on('input onkeypress paste', 'input[data-phone-hotline]', function () {
        $(this).val($(this).val().replace(/[^0-9]/g, ''));
        if ($(this).val().length > 10) {
            $(this).val($(this).val().slice(0, 10));
        }
    })
    $(document).on('focusout', 'input[data-phone-hotline]', function () {
        removeErrorInput($(this));
    })
    ...
    $(document).on('input paste keyup', 'input[data-special]', function () {
        // Valid characters  - + & ( ) % / _
        $(this).val($(this).val().replace(/[`~!@#$^*|\=?;:'",.<>\[\]\\]/gi, ''));
    })

    $(document).on('input paste keyup', 'input[data-spec-display-second]', function () {
        // Valid characters  - + & ( ) % / . , ! _
        $(this).val($(this).val().replace(/[`~@#$^*|\=?;:'"<>{}\[\]\\]/gi, ''));
    })
...
function checkValidateSave(el) {
    ...
    el.find('input:not(:checkbox)').each(function () {
        if ($(this).attr('type') != 'file') {
            ...
            if ($(this).parents('.d-none').length === 0 && !$(this).hasClass('disabled')) {
                ...
                if ($(this).attr('data-phone-hotline')) {
                    let rePhone = /^(09|03|07|08|05|04|06|02|01|1).*$/;
    ...
    el.find('textarea[data-note-textarea]').each(function () {
        $(this).val($(this).val().trim());
        if ($(this).parents('.d-none').length === 0 && !$(this).hasClass('disabled')) {
            $(this).parents('.form-group').find('.error').remove();
            $(this).parent().removeClass('validate-error');
            $(this).parent().removeClass('focus-validate');
            let element = $(this).attr('data-validate');
            if ($(this).attr('data-note-textarea')) {
                if ($(this).val() === '' || $(this).val().length < 2) {
                    // $(this).parent().addClass('validate-error');
                    text = 'Tối thiểu 2 đến 500 ký tự';
                    addErrorInput($(this).parent(), text);
                    flag = false;
                } else {
                    $(this).parent().removeClass('validate-error');
